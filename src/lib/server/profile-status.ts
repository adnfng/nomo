export type ProfileStatus = 'live' | 'missing' | 'unknown';
...
async function probe(username: string, fetcher: typeof fetch): Promise<ProfileStatus> {
  let failed = false;
  ...
  return failed ? 'unknown' : 'missing';
}

export function createProfileStatus({ fetcher = fetch, now = Date.now, ttl = 60_000, always = [] }: Options = {}) {
  ...
  return function status(username: string): Promise<ProfileStatus> {
    ...
    const next = probe(username, fetcher);
